  ...
  async function oauth(provider: 'google' | 'github') {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          // This MUST match what is in Supabase Auth > URL Configuration > Redirect URLs
        ...
        <form onSubmit={handleEmail} style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:6 }}>
              ...
              <Link href="/forgot-password" style={{ fontSize:12, color:'var(--violet2)', textDecoration:'none' }}>Forgot?</Link>
            </div>
            <input className="inp" type="password" placeholder="••••••••" value={pass} onChange={e=>setPass(e.target.value)} required autoComplete="current-password" />
          </div>
